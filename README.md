...
### 1. (Optional) Create the KV namespace for the Next.js cache

Cloudflare Workers have no filesystem, so if you want the Next.js
data/route cache (ISR, fetch caching) to persist across requests, it's
backed by Workers KV instead. **This is not required for your first
deploy** — `wrangler.jsonc` ships with the KV binding commented out, and
`open-next.config.ts` falls back to an in-memory cache. Come back to this
once the app is live if you start using `revalidate`/ISR.
...
Copy the `id` it prints, then in `wrangler.jsonc` uncomment the
`kv_namespaces` block and paste it in, and in `open-next.config.ts`
uncomment the `kvIncrementalCache` import and option.
...
3. In **Settings → Build**, set:
   ...
   > **Important:** don't split this into separate "Build command" /
   > "Deploy command" values (e.g. `npx opennextjs-cloudflare build` +
   > `npx wrangler deploy`) even though the framework preset may suggest
   > that shape. Cloudflare's Git-connected Workers builds have a known
   > failure mode where the deploy step runs without reliably waiting on
   > the build step, producing `[ERROR] The entry-point file at
   > ".open-next/worker.js" was not found.` Putting both steps in one
   > script (`cf:deploy` in `package.json`, which runs
...
If you still hit the `.open-next/worker.js was not found` error after
this, it almost always means the build step itself failed before
producing output (most commonly: missing `NEXT_PUBLIC_*` build variables,
or the KV namespace placeholder below not being replaced) — check the
full build log rather than just the final line.
